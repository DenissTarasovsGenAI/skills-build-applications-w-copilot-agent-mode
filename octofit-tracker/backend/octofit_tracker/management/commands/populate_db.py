...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Create workouts
        Workout.objects.create(name='Morning Cardio', description='Cardio session', duration=40)
        Workout.objects.create(name='Strength Training', description='Weights and resistance', duration=60)

        # Create leaderboard
        Leaderboard.objects.create(user=tony, score=1200)
        Leaderboard.objects.create(user=steve, score=1100)
        Leaderboard.objects.create(user=bruce, score=1300)
        Leaderboard.objects.create(user=clark, score=1250)
